...
def inject_data():
    ...
    for record in movie_list:
        genre_list = []
        for category in record["genre"]:
            category = category.strip()
            if not Genre.objects.filter(category__iexact=category).exists():
                obj = Genre.objects.create(category=category)
            else:
                obj = Genre.objects.get(category__iexact=category)
            genre_list.append(obj)
        mov_obj = Movies.objects.create(name=record['name'].strip(),
                             director=record['director'].strip(),
                             popularity=record['99popularity'],
                             imdb_score=record['imdb_score']
                            )
        if genre_list:
            mov_obj.genre.add(*genre_list)
        print "{} Movie create".format(mov_obj.name)
